/**
 * HOSE360 Step 4 — End 1 METRIC fitting shape+size picker
 */
...
function End1MetricInner({ options }: { options: Hose360Options }) {
  ...
  return (
    <Hose360Layout currentStep={4} totalSteps={11} showPriceBar={false}>
      <FittingFamilyPicker
        familyData={options.fittingFamilies.metric}
        hoseSize={config.selectedHose.size}
        end={1}
        onPicked={(shape, size, price) => {
          setEnd1Fitting(shape, size, price, 'end1-metric');
...
export default function End1Metric() {
  return <Hose360OptionsGate>{(options) => <End1MetricInner options={options} />}</Hose360OptionsGate>;
}
