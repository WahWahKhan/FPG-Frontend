/**
 * HOSE360 Step 4 — End 1 BSP fitting shape+size picker
 */
...
function End1BspInner({ options }: { options: Hose360Options }) {
    ...
    <Hose360Layout currentStep={4} totalSteps={11} showPriceBar={false}>
      <FittingFamilyPicker
        ...
        onPicked={(shape, size, price) => {
          setEnd1Fitting(shape, size, price, 'end1-bsp');
...
export default function End1Bsp() {
  return <Hose360OptionsGate>{(options) => <End1BspInner options={options} />}</Hose360OptionsGate>;
}
