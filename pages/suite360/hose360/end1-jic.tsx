/**
 * HOSE360 Step 4 — End 1 JIC fitting shape+size picker
 */
...
function End1JicInner({ options }: { options: Hose360Options }) {
  ...
  return (
    <Hose360Layout currentStep={4} totalSteps={11} showPriceBar={false}>
      <FittingFamilyPicker
        familyData={options.fittingFamilies.jic}
        hoseSize={config.selectedHose.size}
        end={1}
        onPicked={(shape, size, price) => {
          setEnd1Fitting(shape, size, price, 'end1-jic');
...
export default function End1Jic() {
  return <Hose360OptionsGate>{(options) => <End1JicInner options={options} />}</Hose360OptionsGate>;
}
