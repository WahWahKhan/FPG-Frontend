/**
 * HOSE360 Step 6 — End 2 ORFS fitting shape+size picker.
...
function End2OrfsInner({ options }: { options: Hose360Options }) {
    ...
    <Hose360Layout currentStep={6} totalSteps={11} showPriceBar={false}>
      ...
      <FittingFamilyPicker
        ...
        onPicked={(shape, size, price) => {
          setEnd2Fitting(shape, size, price, 'end2-orfs');
...
export default function End2Orfs() {
  return <Hose360OptionsGate>{(options) => <End2OrfsInner options={options} />}</Hose360OptionsGate>;
}
