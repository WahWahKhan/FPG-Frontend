/**
 * HOSE360 Step 6 — End 2 METRIC fitting shape+size picker.
...
function End2MetricInner({ options }: { options: Hose360Options }) {
    ...
    <Hose360Layout currentStep={6} totalSteps={11} showPriceBar={false}>
      ...
      <FittingFamilyPicker
        ...
        onPicked={(shape, size, price) => {
          setEnd2Fitting(shape, size, price, 'end2-metric');
...
export default function End2Metric() {
  return <Hose360OptionsGate>{(options) => <End2MetricInner options={options} />}</Hose360OptionsGate>;
}
