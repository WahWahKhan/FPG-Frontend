/**
 * HOSE360 Step 6 — End 2 JIC fitting shape+size picker.
...
function End2JicInner({ options }: { options: Hose360Options }) {
    ...
    <Hose360Layout currentStep={6} totalSteps={11} showPriceBar={false}>
      ...
      <FittingFamilyPicker
        ...
        onPicked={(shape, size, price) => {
          setEnd2Fitting(shape, size, price, 'end2-jic');
...
export default function End2Jic() {
  return <Hose360OptionsGate>{(options) => <End2JicInner options={options} />}</Hose360OptionsGate>;
}
