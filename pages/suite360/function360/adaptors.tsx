/**
 * FUNCTION360 Adaptors Page - Step 4
 * Select or skip adaptors component
 */
...
function AdaptorsInner({ options }: { options: any }) {
  const componentDataFile = options.components.adaptors;
  ...
  useEffect(() => {
    setIsSelected(config.selectedComponents.adaptors);  // ← Keep component key
  }, [config.selectedComponents.adaptors]);
  ...
  return (
    <Function360Layout currentStep={4} totalSteps={9}>
...
export default function Adaptors() {
  return <Function360OptionsGate>{(options) => <AdaptorsInner options={options} />}</Function360OptionsGate>;
}